...
describe("throttle", { timeout: 1000 }, () => {
	it.scoped("should execute callback and return result", () =>
		Effect.gen(function* () {
	...
	it.scoped("should throttle rapid calls", () =>
		Effect.gen(function* () {
	...
	it.scoped("should maintain argument-result correlation", () =>
		Effect.gen(function* () {
			const throttled = yield* throttle(
				(x: string) =>
	...
	it.scoped("should propagate errors correctly", () =>
		Effect.gen(function* () {
			const throttled = yield* throttle(
	...
	it.scoped("should handle cleanup and interrupt pending calls", () =>
		Effect.gen(function* () {
	...
	it.scoped("should process calls in FIFO order", () =>
		Effect.gen(function* () {
			const order: number[] = [];
			const throttled = yield* throttle(
				(x: number) =>
