...
test("replicant - should do nothing when Logger._shouldLogReplicants is false", ({ logger, }) => {
    const info = vi.spyOn(console, "info");
    logger.replicants("replicants");
    expect(info).not.toBeCalled();
});
test("logging methods should all do nothing when _silent is true", ({ logger, }) => {
    Logger._silent = true;
...
test("logging methods should all do nothing when the log level is above them", ({ logger, }) => {
...
test("logging methods should all prepend the instance name to the output", ({ logger, }) => {
    Logger._level = LogLevel.Trace;
    Logger._silent = false;
    const info = vi.spyOn(console, "info");
    // Trace
...
test("Sentry - should log errors to Sentry when global.sentryEnabled is true", ({ sentryLogger, SentryMock, }) => {
    ...
    expect(SentryMock.captureException.mock.calls[0][0].message).toMatchInlineSnapshot(`"[sentryClient] error message"`);
});
test("Sentry - should prettyprint objects", ({ sentryLogger, SentryMock }) => {
    sentryLogger.error("error message:", { foo: { bar: "baz" } });
    expect(SentryMock.captureException.mock.calls[0][0].message).toMatchInlineSnapshot(`"[sentryClient] error message: { foo: { bar: 'baz' } }"`);
});
