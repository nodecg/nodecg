...
export class CommandService extends Effect.Service<CommandService>()(
	"CommandService",
	{
		effect: Effect.fn("CommandService.make")(function* () {
			return {
				exec: (cmd: string, args: ReadonlyArray<string>, options?: { cwd?: string }) =>
					Effect.fn("exec")(function* () {
				...
				string: (cmd: string, args: ReadonlyArray<string>, options?: { cwd?: string }) =>
					Effect.fn("string")(function* () {
						let command = PlatformCommand.make(cmd, ...args);
						if (options?.cwd) {
							command = PlatformCommand.workingDirectory(command, options.cwd);
						}
