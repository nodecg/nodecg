...
export const soundsRouter = Effect.fn("soundsRouter")(function* (
	...
	bundles.forEach((bundle) => {
		...
		if (bundle.soundCues.length > 0) {
			...
			const defaultCuesRepValue = makeCuesRepDefaultValue(bundle);
	...
	function serveDefault(
		...
		const bundle = bundles.find((b) => b.name === req.params.bundleName);
		if (!bundle) {
	...
	function makeCuesRepDefaultValue(bundle: NodeCG.Bundle): NodeCG.SoundCue[] {
	...
	app.get("/sound/:bundleName/:cueName/default.mp3", serveDefault);
	app.get("/sound/:bundleName/:cueName/default.ogg", serveDefault);
