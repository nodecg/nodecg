...
test('when there is no "dashboard" folder, assign an empty array to bundle.dashboard.panels', () => {
	const parsedBundle = parseBundle(
		testDirPath("fixtures/bundle-parser/no-panels"),
	);
...
test('when there is a "dashboard" folder but no "dashboardPanels" property, throw an error', () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/no-panels-prop"),
		),
...
test('when critical properties are missing from the "dashboardPanels" property, throw an error explaining what is missing', () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/missing-panel-props"),
		),
	).toThrowErrorMatchingInlineSnapshot(
...
test("when two panels have the same name, throw an error", () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/dupe-panel-name"),
		),
	).toThrowErrorMatchingInlineSnapshot(
...
test("when a panel's file has no <!DOCTYPE>, throw an error", () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/no-doctype"),
		),
	).toThrowErrorMatchingInlineSnapshot(
...
test("when a panel's file has a BOM before it's <!DOCTYPE>, continue to parse it", () => {
	const parsedBundle = parseBundle(
		testDirPath("fixtures/bundle-parser/bom-doctype"),
	);
...
test("when a panel's file does not exist, throw an error", () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/non-existant-panel"),
		),
	).toThrowErrorMatchingInlineSnapshot(
...
test("when a dialog has a workspace, throw an error", () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/dialog-workspace"),
		),
	).toThrowErrorMatchingInlineSnapshot(
...
test("when a dialog is fullbleed, throw an error", () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/dialog-fullbleed"),
		),
	).toThrowErrorMatchingInlineSnapshot(
...
test("when a fullbleed panel has a workspace, throw an error", () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/fullbleed-workspace"),
		),
	).toThrowErrorMatchingInlineSnapshot(
...
test("when a fullbleed panel has a defined width, throw an error", () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/fullbleed-width"),
		),
	).toThrowErrorMatchingInlineSnapshot(
...
test("when a panel has a workspace that begins with __nodecg, throw an error", () => {
	expect(
		parseBundle.bind(
			parseBundle,
			testDirPath("fixtures/bundle-parser/reserved-workspace__nodecg"),
		),
	).toThrowErrorMatchingInlineSnapshot(
