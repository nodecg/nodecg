...
test.serial(
	"should reject acknowledgement promises if there was an error",
	async (t) => {
...
test.serial(
	"should not return a promise if the user provided a callback ",
	async (t) => {
...
test.serial(
	"should prevent acknowledgements from being called more than once",
	async (t) => {
		t.plan(3);
		let callbacksInvoked = 0;
...
test.serial("server - should support intra-context messaging", async (t) => {
	...
	const response = await dashboard.evaluate(
		() => (window as any)._serverToServerData,
	);
...
test("bundles replicant", (t) => {
	const bundlesRep = t.context.apis.extension.Replicant<NodeCG.Bundle[]>(
		"bundles",
		"nodecg",
	);
