...
test('should serve bundle-specific bower_components', async (t) => {
	const response = await fetch(`${C.bundleBowerComponentsUrl()}confirmation.js`);
	t.is(response.status, 200);
	t.is(await response.text(), "const confirmed = 'bower_components_confirmed';\n");
});
...
test('should 404 on non-existent bower_component', async (t) => {
	const response = await fetch(`${C.bundleBowerComponentsUrl()}confirmation_404.js`);
	t.is(response.status, 404);
});

test('should 404 on non-existent node_module', async (t) => {
	const response = await fetch(`${C.bundleNodeModulesUrl()}confirmation_404.js`);
	t.is(response.status, 404);
});

test('should 404 on non-existent bundle node_modules/bower_components', async (t) => {
	const response = await fetch(`${C.rootUrl()}bundles/false-bundle/node_modules/confirmation_404.js`);
	t.is(response.status, 404);
});
...
test.serial('shared sources - 200', async (t) => {
	const response = await fetch(`${C.rootUrl()}bundles/test-bundle/shared/util.js`);
	t.is(response.status, 200);
	t.is(await response.text(), 'window.SharedUtility = {\n\tsomeFunc() {},\n};\n');
});

test('shared sources - 404', async (t) => {
	const response = await fetch(`${C.rootUrl()}bundles/test-bundle/shared/404.js`);
	t.is(response.status, 404);
});

test('shared sources - no bundle 404', async (t) => {
	const response = await fetch(`${C.rootUrl()}bundles/false-bundle/shared/404.js`);
	t.is(response.status, 404);
});
