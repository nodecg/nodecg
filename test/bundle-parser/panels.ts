...
test('when there is a "dashboard" folder but no "dashboardPanels" property, throw an error', (t) => {
	...
	return t.true(error.message.includes('no "nodecg.dashboardPanels" property was found'));
});
...
test('when critical properties are missing from the "dashboardPanels" property, throw an error explaining what is missing', (t) => {
	...
	if (!error) return t.fail();
	return t.true(error.message.includes('the following properties: name, title, file'));
});
...
test('when a dialog has a workspace, throw an error', (t) => {
	...
	if (!error) return t.fail();
	return t.true(error.message.includes("Dialogs don't get put into workspaces"));
});
...
test('when a fullbleed panel has a workspace, throw an error', (t) => {
	...
	if (!error) return t.fail();
	return t.true(error.message.includes('Fullbleed panels are not allowed to define a workspace'));
});
...
test('when a fullbleed panel has a defined width, throw an error', (t) => {
	...
	if (!error) return t.fail();
	return t.true(error.message.includes('Fullbleed panels have their width set based on the'));
});
...
test('when a panel has a workspace that begins with __nodecg, throw an error', (t) => {
	...
	if (!error) return t.fail();
	return t.true(error.message.includes('whose name begins with __nodecg, which is a reserved string'));
});
