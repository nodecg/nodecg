...
test('should error when package.json does not exist', (t) => {
	...
	return t.true(error.message.includes('does not contain a package.json!'));
});
...
test('should error when package.json has no "nodecg" property', (t) => {
	...
	if (!error) return t.fail();
	return t.true(error.message.includes('lacks a "nodecg" property, and therefore cannot be parsed'));
});
...
test('should error when package.json is not valid JSON', (t) => {
	...
	if (!error) return t.fail();
	return t.true(error.message.includes('package.json is not valid JSON'));
});
...
test('should error when "nodecg.compatibleRange" is not a valid semver range', (t) => {
	...
	if (!error) return t.fail();
	return t.true(error.message.includes(''));
});
