...
test.serial(
	'should emit a "change" event after successful declaration when the value is undefined',
	async (t) => {
		const ret = await dashboard.evaluate(
			async () =>
...
test.serial(
	'when an array - should support the "delete" operator',
	async (t) => {
		const ret = await dashboard.evaluate(
			async () =>
				new Promise<{
...
test.serial(
	"when an object - should not cause server-side replicants to lose observation",
	async (t) => {
		t.plan(2);
		setTimeout(() => {
			t.fail("Timeout");
...
test.serial(
	"when an object - should react to changes in nested properties",
	async (t) => {
		...
		await dashboard.evaluate(
			async () =>
				new Promise<void>((resolve) => {
					const rep =
						window.dashboardApi.Replicant<RepType>("s2c_nestedArrTest");
					rep.on("declared", () => {
						resolve();
						rep.on("change", (newVal, oldVal, operations) => {
...
test.serial(
	"when an object - should properly proxy new objects assigned to properties",
	async (t) => {
		t.plan(1);

		const rep = t.context.apis.extension.Replicant<Record<string, any>>(
			"serverObjProp",
			{
...
test.serial(
	"when a date - should emit the JSON value to clients",
	async (t) => {
		const date = new Date();
