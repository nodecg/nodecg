...
test.serial(
	"should return a reference to any already-declared replicant",
	(t) => {
...
test.serial(
	"should be readable without subscription, via readReplicant",
	(t) => {
...
test.serial("should throw an error when no name is provided", (t) => {
	...
	return t.true(
		error.message.includes("Must supply a name when instantiating a Replicant"),
	);
});
...
test.serial(
	"should react to changes in nested properties of objects",
	async (t) => {
		t.plan(3);
...
test.serial(
	"should only apply array splices from the client once",
	async (t) => {
		t.plan(3);
			...
			.evaluate(
				async () =>
					new Promise<void>((resolve) => {
						(window as any).clientDoubleApplyTest =
							window.dashboardApi.Replicant("clientDoubleApplyTest");
...
test.serial(
	'should not override/quickfire .once for events other than "change"',
	(t) => {
		const rep = t.context.apis.extension.Replicant(
			"serverNotOverrideOtherOnceListeners",
			{
				persistent: false,
			},
		);
...
test.serial(
	"objects - throw an error when an object is owned by multiple Replicants",
	(t) => {
		...
		if (!error) return t.fail();
		return t.true(
			error.message.startsWith("This object belongs to another Replicant"),
		);
	},
);
...
test.serial(
	"persistent - should load persisted values when they exist",
	async (t) => {
...
test.serial("test that one else path that's hard to hit", (t) => {
	const rep = t.context.apis.extension.Replicant<boolean[]>(
...
test.serial(
	"provides accurate new and old values for assignment operations",
	async (t) => {
		t.plan(8);

		const rep = t.context.apis.extension.Replicant<number | undefined>(
...
test.serial(
	"should not emit more than one change event on startup when a defaultValue is supplied",
	async (t) => {
		t.plan(1);

		const rep = t.context.apis.extension.Replicant(
			"multipleStartupChangeEvents",
			{ defaultValue: 0 },
		);
...
test.serial(
	"should force persistence after maximumTimeAReplicantCanGoWithoutSaving",
	async (t) => {
		t.plan(1);
