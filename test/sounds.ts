...
test.serial(
	"soundCues replicant - should generate the correct defaultValue",
	(t) => {
...
test.serial(
	"soundCues replicant - should remove any persisted cues that are no longer in the bundle manifest",
	(t) => {
		const rep = t.context.apis.extension.Replicant(
			"soundCues",
			"remove-persisted-cues",
		);
		t.deepEqual(rep.value, [
			{
...
test.serial(
	"mixer - assignable cues - should list new sound Assets as they are uploaded",
	async (t) => {
		/*
	 ...
	 */
		await new Promise<void>((resolve, reject) => {
			const oggPath = path.join(
				C.assetsRoot(),
				"test-bundle/sounds/success.ogg",
			);
			fs.copy(
...
test.serial(
	'client api - should emit "ncgSoundsReady" once all the sounds have loaded',
	async (t) => {
...
test.serial(
	"client api - #stopSound should stop all instances of a cue",
	async (t) => {
		const ret = await graphic.evaluate(() => {
...
test.serial(
	"client api - #stopAllSounds should stop all instances",
	async (t) => {
		const ret = await graphic.evaluate(() => {
			window.graphicApi.playSound("default-file");
			window.graphicApi.playSound("default-file");
