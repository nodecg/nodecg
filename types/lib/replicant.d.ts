...
export class ReplicantServer<V> extends ReplicantCommon<V> {
	...
	value: V;
	namespace?: string;
	on(event: 'change', listener: (newValue: V, oldValue?: V, operationQueue?: OperationQueueItem[]) => void): this;
	[prop: string]: any;
}

export class ReplicantBrowser<V> extends ReplicantCommon<V> {
	static declareReplicants(): DeclaredReplicants<'browser'>;
	constructor(name: string, namespace: string, opts: ReplicantOptions<V>, socket: SocketIOClient.Socket);
	value?: V;
	namespace: string;
...
export type Replicant<V, P extends Platform> = P extends 'browser' ? ReplicantBrowser<V> : ReplicantServer<V>;
