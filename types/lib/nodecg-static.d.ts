import { Replicant, ReplicantOptions, DeclaredReplicants, ReplicantBrowser } from './replicant';
 ...
 */
export type NodeCGStatic<P extends Platform> = P extends 'server' ? NodeCGStaticServer : NodeCGStaticBrowser;

/**
 * sendMessage returns Promise in browser, void in extensions
 */
type SendMessageReturnType<P extends Platform> = P extends 'server' ? void : Promise<any>;

/**
 * Combined 'sendMessageToBundle' method for browser and extensions
 */
interface SendMessageToBundle<P extends Platform, M = SendMessageReturnType<P>> {
	(messageName: string, bundleName: string, cb?: (error: any, ...args: any[]) => void): M;
	(messageName: string, bundleName: string, data: any, cb?: (error: any, ...args: any[]) => void): M;
}
