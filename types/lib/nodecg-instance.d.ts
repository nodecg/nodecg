...
interface NodeCGCommon<P extends Platform, M = SendMessageReturnType<P>> {
	...
	sendMessage(messageName: string, cb?: (error: any, ...args: any[]) => void): M;
	sendMessage(messageName: string, data: any, cb?: (error: any, ...args: any[]) => void): M;
...
export interface NodeCGBrowser extends NodeCGCommon<'browser'> {
	...
	soundReady?: boolean;
	getDialog(name: string, bundle?: string): ReturnType<ParentNode['querySelector']>;
 ...
 */
export type NodeCG<P extends Platform> = P extends 'server' ? NodeCGServer : NodeCGBrowser;
