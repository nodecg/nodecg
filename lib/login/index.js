    ...
    passport.use(new SteamStrategy({
        ...
        function(identifier, profile, done) {
            process.nextTick(function() {
                ...
                profile.allowed = (config.login.steam.allowedIds.indexOf(profile.identifier) > -1);

                if (profile.allowed) {
                    log.info("[lib/login/index.js] Granting %s access", profile.identifier);
                } else {
                    log.info("[lib/login/index.js] Denying %s access", profile.identifier);
                }
