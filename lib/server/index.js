...
module.exports.start = function ({portOverride} = {}) {
	...
	if (typeof portOverride === 'number') {
		log.warn(`Port override ${portOverride} provided, using that port instead of ${config.port}`);
		configHelper.config.port = portOverride;
		configHelper.filteredConfig.port = portOverride;
	}
