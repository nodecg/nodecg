...
module.exports.start = function () {
	...
	const bundleRootPaths = (() => {
		if (process.env.BUNDLE_ROOT) {
			return null;
		}

		const bundleRootPaths = [path.join(process.env.NODECG_ROOT, 'bundles')].concat(config.bundles.paths);
		if (global.isZeitPkg) {
			bundleRootPaths.unshift(path.resolve(__dirname, '../../bundles'));
		}

		return bundleRootPaths;
	})();

	const bundlePaths = process.env.BUNDLE_ROOT ? [process.env.BUNDLE_ROOT] : null;
	...
	const nodecgVersion = pjson.version;
	bundleManager.init({bundlePaths, bundleRootPaths, cfgPath, nodecgVersion, config, Logger});
