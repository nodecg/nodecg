...
if (config.rollbar && config.rollbar.enabled) {
	...
	rollbar.handleUncaughtExceptionsAndRejections(config.rollbar.postServerItem, {
		exitOnUncaughtException: config.exitOnUncaught
	});
	global.rollbarEnabled = true;
	console.info('[nodecg] Rollbar enabled.');
}
...
module.exports.start = function () {
	...
	app = express();
	app.use(favicon('lib/dashboard/public/img/favicon.ico'));
	...
	bundleManager.init(process.cwd(), require('../../package.json').version, config, Logger).then(() => {
	...
	io.on('error', err => {
		if (global.rollbarEnabled) {
			require('rollbar').handleError(err);
		}
