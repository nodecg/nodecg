...
module.exports = function (pathOrHtml, resourceType, opts, cb) {
	...
	function inject(err, html) {
		...
		if (resourceType === 'panel' || resourceType === 'dialog') {
			...
			if (opts.sound) {
				scripts.push('<script src="/bower_components/SoundJS/lib/soundjs.min.js"></script>');
			}
		...
		if (opts.createApiInstance) {
			const fullBundle = bundles.find(opts.createApiInstance.name);
			const partialBundle = {
				name: opts.createApiInstance.name,
				config: opts.createApiInstance.config,
				version: fullBundle ? fullBundle.version : undefined,
				git: fullBundle ? fullBundle.git : undefined,
