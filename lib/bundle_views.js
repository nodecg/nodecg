...
var tokens = config.login.enabled ? require('./login/tokens') : null;

require('string.prototype.endswith');
log.trace('Adding Express routes');
app.set('views', path.resolve(__dirname, '..'));

app.get('/view/:bundleName*', checkKey, function(req, res, next) {
    ...
    if (resName.endsWith('.html')) {
        ...
        if (config.login.enabled) {
            scripts[2] = '<script>var socket = io.connect("//%s/", { query: "token=" + qs["key"] });';
...
app.get('/view/:bundle/components/*', checkKey, function(req, res, next) {
...
function checkKey(req, res, next) {
    ...
    tokens.find({ token: req.query.key }, function(err, token) {
        ...
        if (token) {
            ...
            res.render('./lib/login/public/authError.jade', {
                message: 'Unauthorized',
                code: 'invalid_token',
                viewUrl: req.originalUrl.split('?')[0]
            });
        }
    });
}
