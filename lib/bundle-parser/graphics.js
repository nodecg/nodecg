...
module.exports = function(graphicsDir, bundle) {
	...
	if (fs.existsSync(graphicsDir) && typeof bundle.graphics === 'undefined') {
		throw new Error(
			`${bundle.name} has a "graphics" folder, ` +
	...
	if (!fs.existsSync(graphicsDir) && typeof bundle.graphics !== 'undefined') {
		throw new Error(
			`${bundle.name} has a "nodecg.graphics" property in its package.json, but no "graphics" folder`,
		);
	}
	...
	bundle.graphics.forEach((graphic, index) => {
		...
		if (dupeFound) {
			throw new Error(
				`Graphic #${index} (${graphic.file}) has the same file as another graphic in ${bundle.name}`,
			);
		}
