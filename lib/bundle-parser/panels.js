...
module.exports = function(dashboardDir, bundle) {
	...
	if (fs.existsSync(dashboardDir) && typeof bundle.dashboardPanels === 'undefined') {
		throw new Error(
			`${bundle.name} has a "dashboard" folder, ` +
	...
	if (!fs.existsSync(dashboardDir) && typeof bundle.dashboardPanels !== 'undefined') {
		throw new Error(
			`${bundle.name} has a "nodecg.dashboardPanels" property in its package.json, ` +
	...
	bundle.dashboardPanels.forEach((panel, index) => {
		...
		if (!html.match(/(<!doctype )/gi)) {
			throw new Error(
				`Panel "${path.basename(panel.file)}" in bundle "${bundle.name}" has no DOCTYPE,` +
		...
		if (panel.dialog && panel.workspace) {
			throw new Error(
				`Dialog "${path.basename(panel.file)}" in bundle "${bundle.name}" has a "workspace" ` +
		...
		if (panel.dialog && panel.fullbleed) {
			throw new Error(
				`Panel "${path.basename(panel.file)}" in bundle "${bundle.name}" is fullbleed, ` +
		...
		if (!panel.dialog) {
			...
			if (panel.workspace.startsWith('__nodecg')) {
				throw new Error(
					`Panel "${path.basename(panel.file)}" in bundle "${bundle.name}" is in a workspace ` +
