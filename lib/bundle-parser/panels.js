...
const schema = require('../../schemas/dashboard-panel.json');

function getParsingErrors(errors) {
	const err = [];
	errors.forEach(e => {
		err.push(e.message);
	});
...
module.exports = function (dashboardDir, bundle) {
	...
	bundle.dashboardPanels.forEach((panel, index) => {
		...
		if (!result) {
			console.log(getParsingErrors(validate.errors));
			throw new Error(`Parsing Panel #${index} failed because of the following errors:\n${getParsingErrors(validate.errors)}`);
		}
