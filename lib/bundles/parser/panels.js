...
function readDashboardPanels(bundle) {
    ...
    manifest.forEach(function(panel, index) {
        try {
            ...
            // Check if this bundle already has a panel with the same element
            var dupeFound = bundle.dashboard.panels.some(function(p) {
                return p.element === panel.element;
            });
            if (dupeFound) {
                log.error('[%s] Panel #%d (%s) has the same element as another panel in this bundle, and will not be loaded.',
