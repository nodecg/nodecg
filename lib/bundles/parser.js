...
Parser.prototype.readDashboardResources = function(bundle) {
    ...
    dashboardDir.forEach(function(file) {
        ...
        // Panels are strictly limited to Jade, JS, and CSS. No EJS, HTML, LESS, SASS, etc.
        switch (path.extname(filepath)) {
            case '.jade':
                try {
                    // Render the Jade, giving it access to both this specific bundle's config and NodeCG's config
                    bundle.dashboard.panels.push(jade.renderFile(filepath, {
                        bundleConfig: bundle.config,
                        ncgConfig: config
                    }));
                } catch (e) {
                    log.error("[%s] Error when rendering %s:\n", bundle.name, file, e.message);
                }
                break;
            case '.js':
...
Parser.prototype.readConfig = function(bundle) {
    var cfgPath = path.resolve(process.cwd(), 'cfg', bundle.name + '.json');
