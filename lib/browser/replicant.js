...
const REPLICANT_HANDLER = {
	...
	set(target, prop, newValue) {
		...
		if (target.status !== 'declared') {
			target._queueAction(target._doBrowserSetter, [newValue]);
			return;
		}

		target._doBrowserSetter(newValue);
	}
};
...
class Replicant extends EventEmitter {
	...
	constructor(name, bundle, opts, socket) {
		...
		socket.on('reconnect', () => this._doBrowserDeclare());
	...
	/**
	 * Adds an operation to the operation queue, to be flushed at the end of the microtask.
	...
	_flushOperations() {
		...
		this._socket.emit('replicant:proposeOperations', {
			...
			this._assignValue(value, revision);
		});
	 ...
	 */
	_processActions() {
		this._actionQueue.forEach(item => item.fn.apply(this, item.args));
		this._actionQueue = [];
	}
	...
	_assignValue(newValue, revision) {
		// TODO: does this need the equivalent of "unobserveValue"?
		const oldValue = clone(this.value);

		this.value = shared._proxyRecursive(newValue, '/');
	...
	_handleAssignment(data) {
		...
		this.log.replicants('replicantAssigned', data);

		if (data.originatorId === this.id) {
			this.emit('assignmentAccepted', data);
		}
	...
	_handleOperations(data) {
		...
		if (data.name !== this.name || data.bundle !== this.bundle) {
			return;
		} else if (data.revision !== (this.revision + 1)) {
			this.log.warn('Not at head revision (ours %s, theirs %s), fetching latest...',
				this.revision, data.revision);
			this._fullUpdate();
			return;
		}

		this.log.replicants('replicantOperations', data);
