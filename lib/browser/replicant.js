...
class Replicant extends EventEmitter {
	...
	_proposeAssignment(newValue) {
		this._socket.emit('replicant:proposeAssignment', {
			name: this.name,
			bundle: this.bundle, value: newValue,
			persistent: this.opts.persistent
		});
	}
	...
	_declare() {
		...
		this._socket.emit('joinRoom', this.bundle, () => {
			this._socket.emit('replicant:declare', {
				 ...
				 */
				// TODO: Test to ensure that this doesn't cause proposeAssignment to get emitted.
				if (this.revision !== data.revision || !equal(this.value, data.value)) {
					this._assignValue(data.value, data.revision, true);
	 ...
	 * @param [bypassProxy] {boolean} - If true, bypasses the proxy when setting the newValue.
	 * @private
	 */
	_assignValue(newValue, revision, bypassProxy) {
