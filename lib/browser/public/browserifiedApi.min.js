...
"use strict";function Replicant(e,t,n,a){function i(e,t){g.push({fn:e,args:t})}function s(){g.forEach(function(e){e.fn.apply(this,e.args)})}function r(n){c(n),a.emit("assignReplicant",{name:e,bundle:t,value:n})}function o(i){"declared"!==b.status&&"declaring"!==b.status&&(b.status="declaring",a.emit("joinRoom",t,function(){a.emit("declareReplicant",{name:e,bundle:t,defaultValue:i,persistent:n.persistent},function(e){v.replicants("declareReplicant callback (value: %s, revision: %s)",e.value,e.revision);var t=!1;b.revision===e.revision&&equal(f,e.value)||(p(),f=i,d(),c(e.value,e.revision),t=!0),b.status="declared",b.emit("declared",e),g.length?s():t||b.emit("change",void 0,e.value)})}))}function c(e,t){p();var n=clone(f);f=e,d(),"undefined"!=typeof t&&(b.revision=t),b.emit("change",n,f)}function d(){"object"==typeof f&&null!==f&&Nested.observe(f,u)}function l(n){a.emit("changeReplicant",{name:e,bundle:t,changes:n,revision:b.revision,originatorId:b.id},function(e,t){v.replicants("Not at head revision (ours %s, theirs %s). Change aborted & head revision applied.",b.revision,t),c(e,t)})}function u(e){var t=[];e.forEach(function(e){var n=e.path.substr(1).replace(/\//g,"."),a=objectPath.get(e.root,n);switch(e.type){case"add":t.push({type:"add",path:n,newValue:a});break;case"update":t.push({type:"update",path:n,oldValue:e.oldValue,newValue:a});break;case"splice":t.push({type:"splice",path:n,index:e.index,removed:e.removed,removedCount:e.removed.length,added:e.object.slice(e.index,e.index+e.addedCount),addedCount:e.addedCount});break;case"delete":t.push({type:"delete",path:n,oldValue:e.oldValue});break;default:t.push({type:"other",path:n})}}),"declared"===b.status?l(t):i(l,[t])}function p(){"object"==typeof f&&null!==f&&Nested.unobserve(f,u)}function h(){window.NodeCG.readReplicant(e,t,function(e){b.emit("fullUpdate",e),c(e.value,e.revision)})}if(!e||"string"!=typeof e)throw new Error("Must supply a name when instantiating a Replicant");if(!t||"string"!=typeof t)throw new Error("Must supply a bundle name when instantiating a Replicant");var v=require("./logger")("Replicant/"+t+"."+e);if(declaredReplicants.hasOwnProperty(t)){if(declaredReplicants[t].hasOwnProperty(e))return declaredReplicants[t][e];declaredReplicants[t][e]=this}else declaredReplicants[t]={},declaredReplicants[t][e]=this;var f,b=this;n=n||{},this.name=e,this.bundle=t,this.opts=n,this.revision=0,this.status="undeclared",this.id=uuid.v4(),"undefined"==typeof n.persistent&&(n.persistent=!0);var g=[];c(n.defaultValue),Object.defineProperty(this,"value",{get:function(){return f},set:function(e){return e===f?(v.replicants("value unchanged, no action will be taken"),f):(v.replicants("running setter with",e),"declared"!==b.status?void i(r,[e]):void r(e))},enumerable:!0}),o(n.defaultValue),a.on("replicantAssigned",function(n){n.name===e&&n.bundle===t&&(v.replicants("replicantAssigned",n),b.emit("assignmentAccepted",n),c(n.newValue,n.revision))}),a.on("replicantChanged",function(n){if(n.name===e&&n.bundle===t){if(n.revision!==b.revision+1)return v.replicants('[%s] Replicant "%s" not at head revision (ours %s, theirs %s), fetching latest...',t,e,b.revision,n.revision),void h();v.replicants("replicantChanged",n);var a=clone(f),i=clone(n.changes);i.reverse(),i.forEach(function(e){switch(e.type){case"add":objectPath.del(a,e.path);break;case"update":objectPath.set(a,e.path,e.oldValue);break;case"splice":var t=objectPath.get(a,e.path);t||(t=[]);var n=clone(e.removed);n.unshift(e.addedCount),n.unshift(e.index),Array.prototype.splice.apply(t,n),objectPath.set(a,e.path,t);break;case"delete":objectPath.set(a,e.path,e.oldValue);break;default:objectPath.set(a,e.path,e.oldValue)}}),n.originatorId!==b.id&&(p(),n.changes.forEach(function(e){switch(e.type){case"add":case"update":objectPath.set(f,e.path,e.newValue);break;case"splice":var t=objectPath.get(f,e.path),n=clone(e.added);n.unshift(e.removedCount),n.unshift(e.index),Array.prototype.splice.apply(t,n),objectPath.set(f,e.path,t);break;case"delete":objectPath.del(f,e.path)}}),d()),b.revision=n.revision,b.emit("change",a,f,n.changes)}}),a.on("disconnect",function(){b.status="undeclared"}),a.on("reconnect",function(){o(f)})}var inherits=require("inherits"),EventEmitter=require("events").EventEmitter,Nested=require("nested-observe"),objectPath=require("object-path"),equal=require("deep-equal"),clone=require("clone"),uuid=require("uuid"),declaredReplicants={};inherits(Replicant,EventEmitter),module.exports=Replicant;
