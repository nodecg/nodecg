...
"use strict";function Replicant(e,t,n,a){function i(e,t){g.push({fn:e,args:t})}function r(){var e=this;g.forEach(function(t){return t.fn.apply(e,t.args)})}function s(n){a.emit("assignReplicant",{name:e,bundle:t,value:n,originatorId:v.id})}function o(i){"declared"!==v.status&&"declaring"!==v.status&&(v.status="declaring",a.emit("joinRoom",t,function(){a.emit("declareReplicant",{name:e,bundle:t,defaultValue:i,persistent:n.persistent},function(e){f.replicants("declareReplicant callback (value: %s, revision: %s)",e.value,e.revision);var t=!1;v.revision===e.revision&&equal(b,e.value)||(p(),b=i,d(),c(e.value,e.revision),t=!0),v.status="declared",v.emit("declared",e),g.length>0?r():t||v.emit("change",void 0,e.value)})}))}function c(e,t){p();var n=clone(b);b=e,d(),"undefined"!=typeof t&&(v.revision=t),v.emit("change",n,b)}function d(){"object"===("undefined"==typeof b?"undefined":_typeof(b))&&null!==b&&Nested.observe(b,u)}function l(n){a.emit("changeReplicant",{name:e,bundle:t,changes:n,revision:v.revision,originatorId:v.id},function(e,t){f.replicants("Not at head revision (ours %s, theirs %s). Change aborted & head revision applied.",v.revision,t),c(e,t)})}function u(e){var t=[];e.forEach(function(e){var n=e.path.substr(1).split("/").map(function(e){return e.replace(/~1/g,"/")});1===n.length&&""===n[0]&&(n=[]);var a=objectPath.get(e.root,n);switch(e.type){case"add":t.push({type:"add",path:n,newValue:a});break;case"update":t.push({type:"update",path:n,oldValue:e.oldValue,newValue:a});break;case"splice":t.push({type:"splice",path:n,index:e.index,removed:e.removed,removedCount:e.removed.length,added:e.object.slice(e.index,e.index+e.addedCount),addedCount:e.addedCount});break;case"delete":t.push({type:"delete",path:n,oldValue:e.oldValue});break;default:t.push({type:"other",path:n})}}),"declared"===v.status?l(t):i(l,[t])}function p(){"object"===("undefined"==typeof b?"undefined":_typeof(b))&&null!==b&&Nested.unobserve(b,u)}function h(){window.NodeCG.readReplicant(e,t,function(e){v.emit("fullUpdate",e),c(e.value,e.revision)})}if(!e||"string"!=typeof e)throw new Error("Must supply a name when instantiating a Replicant");if(!t||"string"!=typeof t)throw new Error("Must supply a bundle name when instantiating a Replicant");var f=require("./logger")("Replicant/"+t+"."+e);if(declaredReplicants.hasOwnProperty(t)){if(declaredReplicants[t].hasOwnProperty(e))return declaredReplicants[t][e];declaredReplicants[t][e]=this}else declaredReplicants[t]={},declaredReplicants[t][e]=this;var v=this,b=void 0;n=n||{},this.name=e,this.bundle=t,this.opts=n,this.revision=0,this.status="undeclared",this.id=uuid.v4(),"undefined"==typeof n.persistent&&(n.persistent=!0),this.on("newListener",function(e,t){"change"===e&&"declared"===v.status&&t(void 0,b)});var g=[];Object.defineProperty(this,"value",{get:function(){return b},set:function(e){return e===b?(f.replicants("value unchanged, no action will be taken"),b):(f.replicants("running setter with",e),"declared"!==v.status?void i(s,[e]):void s(e))},enumerable:!0}),o(n.defaultValue),a.on("replicantAssigned",function(n){n.name===e&&n.bundle===t&&(f.replicants("replicantAssigned",n),n.originatorId===v.id&&v.emit("assignmentAccepted",n),c(n.newValue,n.revision))}),a.on("replicantChanged",function(n){if("declared"===v.status&&n.name===e&&n.bundle===t){if(n.revision!==v.revision+1)return f.replicants('[%s] Replicant "%s" not at head revision (ours %s, theirs %s), fetching latest...',t,e,v.revision,n.revision),void h();f.replicants("replicantChanged",n);var a=clone(b),i=clone(n.changes);i.reverse(),i.forEach(function(e){switch(e.type){case"add":objectPath.del(a,e.path);break;case"update":objectPath.set(a,e.path,e.oldValue);break;case"splice":var t=objectPath.get(a,e.path);t||(t=[]);var n=clone(e.removed);n.unshift(e.addedCount),n.unshift(e.index),Array.prototype.splice.apply(t,n),objectPath.set(a,e.path,t);break;case"delete":objectPath.set(a,e.path,e.oldValue);break;default:objectPath.set(a,e.path,e.oldValue)}}),n.originatorId===v.id?n.changes.forEach(function(e){switch(e.type){case"splice":e.object=objectPath.get(b,e.path)}}):(p(),n.changes.forEach(function(e){switch(e.type){case"add":case"update":objectPath.set(b,e.path,e.newValue);break;case"splice":var t=objectPath.get(b,e.path),n=clone(e.added);n.unshift(e.removedCount),n.unshift(e.index),Array.prototype.splice.apply(t,n),objectPath.set(b,e.path,t),e.object=t;break;case"delete":objectPath.del(b,e.path)}}),d()),v.revision=n.revision,v.emit("change",a,b,n.changes)}}),a.on("disconnect",function(){v.status="undeclared"}),a.on("reconnect",function(){return o(b)})}var _typeof="function"==typeof Symbol&&"symbol"==typeof Symbol.iterator?function(e){return typeof e}:function(e){return e&&"function"==typeof Symbol&&e.constructor===Symbol?"symbol":typeof e},inherits=require("inherits"),EventEmitter=require("events").EventEmitter,Nested=require("nested-observe"),objectPath=require("object-path"),equal=require("deep-equal"),clone=require("clone"),uuid=require("uuid"),declaredReplicants={};inherits(Replicant,EventEmitter),module.exports=Replicant,Replicant.declaredReplicants=declaredReplicants;
},{"./logger":3,"clone":10,"deep-equal":11,"events":14,"inherits":16,"nested-observe":17,"object-path":20,"uuid":23}],5:[function(require,module,exports){
