...
function SyncedVariables() {
    ...
    io.sockets.on('connection', function (socket) {
        socket.on('declareVariable', function (data, cb) {
            var returnVal = self.findOrDeclare(data.bundleName, data.variableName, data.initialVal);
            if (typeof cb === 'function')
                cb(returnVal);
        });

        socket.on('assignVariable', function (data, cb) {
            self.assign(data.bundleName, data.variableName, data.value)
        });

        socket.on('destroyVariable', function (data, cb) {
            self.destroy(data.bundleName, data.variableName)
        });
    });
}
