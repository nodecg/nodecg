...
server.on('started', function() {
    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('changeVariable', function (data) {
            exports.digestChanges(data.bundleName, data.changes, data.roots);
        });

        socket.on('readVariable', function (data, cb) {
            var returnVal = exports.findOrDeclare(data.bundleName, data.variableName, data.initialVal);
            if (typeof cb === 'function')
                cb(returnVal);
        });
...
exports.digestChanges = function(bundleName, changes, roots) {
    var changesByRoot = {};
    ...
    changes.forEach(function(change) {
        switch(change.type) {
            case 'add':
            case 'update':
                objectPath.set(declaredVars, change.path, change.newValue);
                break;
            case 'delete':
                objectPath.del(declaredVars, change.path);
                break;
            default:
                // Not sure how to handle prototype changes and whatnot
                break;
        }
        changesByRoot[change.root].push(change);
    });

    for (var root in changesByRoot) {
        if (!changesByRoot.hasOwnProperty(root)) continue;
        this.emitToAll('variableChanged', {
...
exports.declare = function(bundleName, variableName, initialVal) {
    ...
    log.syncedVars('Variable %s (%s) declared:', variableName, bundleName, initialVal);
...
exports.assign = function(bundleName, variableName, value) {
    if (!this.exists(bundleName, variableName)) {
        log.error('Attempted to assign non-existent variable %s (%s)', variableName, bundleName);
        return;
    }

    declaredVars[bundleName][variableName] = value;
    log.syncedVars('Variable %s (%s) assigned: ', variableName, bundleName, value);
...
exports.exists = function(bundleName, variableName) {
    if (!declaredVars.hasOwnProperty(bundleName)) return false;
...
exports.findOrDeclare = function(bundleName, variableName, initialVal) {
    ...
    if (typeof(existingVar) !== 'undefined') {
        log.syncedVars('Variable %s (%s) already existed:', variableName, bundleName, existingVar);
        return existingVar;
    }
...
exports.find = function(bundleName, variableName) {
    ...
    if (!declaredVars.hasOwnProperty(bundleName)) return;

    // If that variable doesn't exist for that bundle, return undefined
    if (!declaredVars[bundleName].hasOwnProperty(variableName)) return;
...
// Only used in tests, use with extreme caution
exports._reset = function() {
    declaredVars = {};
    this.emitToAll('variablesReset');
};
