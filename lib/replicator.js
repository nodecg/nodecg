...
server.on('started', () => {
	io.sockets.on('connection', socket => {
		socket.on('replicant:declare', (data, cb) => {
			...
			try {
				const replicant = findOrDeclare(data.name, data.namespace, data.defaultValue, data.persistent);
		...
		socket.on('replicant:proposeAssignment', (data, cb) => {
			...
			const serverReplicant = findOrDeclare(data.name, data.namespace, undefined, data.persistent);
 ...
 * @param namespace {string} - The namespace to which this Replicant belongs.
 ...
 * @returns {object}
 */
function declare(name, namespace, defaultValue, persistent) {
	...
	return new Replicant(name, namespace, {
		defaultValue,
		persistent
	});
}
 ...
 */
function findOrDeclare(name, namespace, defaultValue, persistent) {
	const existingReplicant = find(name, namespace);
	if (typeof existingReplicant !== 'undefined') {
		return existingReplicant;
	}

	return declare(name, namespace, defaultValue, persistent);
}
...
function emitToClients(namespace, eventName, data) {
	...
	io.to(namespace).emit(eventName, data);
}
