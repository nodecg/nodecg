...
server.on('started', () => {
	io.sockets.on('connection', socket => {
		...
		socket.on('replicant:proposeAssignment', data => {
			...
			assign(serverReplicant, data.value, data.originatorId);
		});

		socket.on('replicant:proposeOperations', (data, cb) => {
			log.replicants('received replicant:proposeOperations', data);
			const serverReplicant = findOrDeclare(data.name, data.bundle, undefined, data.persistent);
			if (serverReplicant && serverReplicant.revision !== data.revision) {
				...
				if (typeof cb === 'function') {
					cb(serverReplicant.value, serverReplicant.revision);
				}
			} else {
				applyOperations(serverReplicant, data.operations, data.originatorId);
			}
		});
...
function declare(name, bundle, defaultValue, persistent) {
	...
	return new Replicant(name, bundle, {defaultValue, persistent});
}
 ...
 */
function assign(replicant, value, originatorId) {
 ...
 * @param originatorId {string} - The ID of the Replicant instance that originated this request.
 */
function applyOperations(replicant, operations, originatorId) {
