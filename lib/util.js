...
exports.authCheck = function (req, res, next) {
    ...
    } else if (req.query.key || req.cookies.socketToken) {
...
exports.injectScripts = function (pathOrHtml, resourceType, opts) {
    opts = opts || {};
    ...
    if (resourceType === 'graphic') {
        var file = fs.readFileSync(pathOrHtml);
        $ = cheerio.load(file);
    } else {
        $ = cheerio.load(pathOrHtml);
    }
    ...
    if (resourceType === 'panel' || resourceType === 'dialog') {
        ...
        if (config.login.enabled) {
            ...
            socketScript =
                '<script>\n' +
                    'var token = qs["key"] || Cookies.get("socketToken");\n' +
                    'var socket = io.connect("//%s/",{query: "token=" + token});\n' +
                '</script>';
        } else {
