    ...
    function inject(err, html) {
        ...
        if (resourceType === 'panel' || resourceType === 'dialog') {
            ...
            if (opts.standalone) {
                ...
                scripts.push("<script>var url = new Url; window.token = url.query.key || Cookies.get('socketToken'); if (window.token) { window.socket = io.connect('//' + window.ncgConfig.baseURL + '/', { query: 'token=' + window.token }); } else { window.socket = io.connect('//' + window.ncgConfig.baseURL + '/'); }</script>");
            }
            else {
