...
var log = require('./logger')('nodecg/lib/util');
...
exports.injectScripts = function(fileLocation, bundle, isDashboard) {
    ...
    if ($head.length < 1) {
        log.error('Panel "%s" in bundle "%s" has no <head>, cannot inject API.',
            path.basename(fileLocation), bundle.name);
        return util.format('Panel "%s" in bundle "%s" has no &lt;head&gt;, cannot inject API.',
            path.basename(fileLocation), bundle.name);
    }
