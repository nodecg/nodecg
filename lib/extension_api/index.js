...
function NodeCG(bundleName, ioInstance) {
    ...
    io.sockets.on('connection', function onConnection(socket) {
        socket.on('message', function onMessage(data, cb) {
            log.trace("[lib/extension_api/index.js] Socket %s sent a message with callback:",
                socket.id, data, cb);
    ...
    syncedVariables.on('variableAssigned', function variableAssigned(data) {
        log.trace("[lib/extension_api/index.js] Variable assigned:", data);
    ...
    syncedVariables.on('variableDestroyed', function variableDestroyed(data) {
        log.trace("[lib/extension_api/index.js] Variable destroyed:", data);
...
NodeCG.prototype.sendMessageToBundle = function (messageName, bundleName, data) {
    log.trace("[lib/extension_api/index.js] Sending message %s to bundle %s with data:",
        messageName, bundleName, data);
...
NodeCG.prototype.listenFor = function (messageName, bundleName, handler) {
    ...
    log.trace("[lib/extension_api/index.js] Listening for %s from bundle %s:",
        messageName, bundleName);
...
NodeCG.prototype.declareSyncedVar = function (args) {
    ...
    log.trace("[lib/extension_api/index.js] Variable declared: name %s bundle %s value %s",
        name, bundle, value);
