...
function Replicant(name, bundle, opts, nodecg) {
    ...
    function replayChange(changes) {
        try {
            ...
            if (inBrowser) {
                ...
                self.revision++;
                replicator.change(name, bundle, changes);
                self.emit('change', oldValue, value, changes);
            }
        } catch (e) {
            log.error(e.stack);
        }
    ...
    function onValueChange(rawChanges) {
        ...
        if (self.status !== 'declared') {
            addToQueue(replayChange, [formattedChanges]);
            return;
        }

        try {
            var oldValue = clone(value);
            var replayChanges = clone(formattedChanges);
