...
function Replicant(name, bundle, opts, nodecg) {
    ...
    function replayChange(rawChanges) {
        try {
            unobserveValue();
            var oldValue = clone(value);
    ...
    function onValueChange(rawChanges) {
        if (self.status !== 'declared') {
            addToQueue(replayChange, [rawChanges]);
            return;
        }

        try {
            var oldValue = clone(value);
            var replayChanges = Array.prototype.slice.call(rawChanges);
            ...
            replayChanges.forEach(function(change) {
                ...
                switch(change.type) {
                    case 'add':
                        objectPath.del(oldValue, path);
                        break;
                    case 'update':
                        objectPath.set(oldValue, path, change.oldValue);
                        break;
                    case 'splice':
                        var arr = objectPath.get(oldValue, path);
                        if (!arr) {
                            arr = [];
                        }
                        ...
                        args.unshift(change.index);
                        Array.prototype.splice.apply(arr, args);
                        objectPath.set(oldValue, path, arr);
                        break;
                    case 'delete':
                        objectPath.set(oldValue, path, change.oldValue);
                        break;
    ...
    function replicantChanged(data) {
        ...
        if (data.revision === self.revision) return;
