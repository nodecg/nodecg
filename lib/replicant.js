...
function Replicant(name, bundle, opts, nodecg) {
    ...
    if (inBrowser) {
        nodecg.socket.emit('joinRoom', bundle, function() {
            nodecg.socket.emit('declareReplicant', {
                ...
                if (self.revision !== data.revision || value !== data.value) {
        ...
        (function() {
            var result = replicator.findOrDeclare(name, bundle, opts.defaultValue);
    ...
    // Listen for `resetAll` event, used only in tests
    if (inBrowser) {
        nodecg.socket.on('resetReplicants', reset);
    } else {
        replicator.on('resetReplicants', reset);
    }

    function reset() {
        unobserveValue();
        for (var key in declaredReplicants) {
            if (!declaredReplicants.hasOwnProperty(key)) continue;
            delete declaredReplicants[key];
        }
        declaredReplicants = {};
    }
}
