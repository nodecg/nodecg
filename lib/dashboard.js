...
const INSTRUMENTED_SRC_PATH = path.join(__dirname, '../instrumented');
...
if (process.env.NODECG_TEST || configHelper.config.developer) {
	app.get('/nodecg-api.min.js', (req, res) => {
		res.sendFile(path.join(BUILD_PATH, 'nodecg-api.min.js'));
	});
	app.get('/nodecg-api.min.js', (req, res) => {
		res.sendFile(path.join(BUILD_PATH, 'nodecg-api.min.js.map'));
	});
	...
	if (process.env.NODECG_TEST) {
		...
		app.get('/*', (req, res, next) => {
			...
			const fp = path.join(INSTRUMENTED_SRC_PATH, resName);
			if (fs.existsSync(fp)) {
