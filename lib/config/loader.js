	...
	const convictSchema = {
		...
		logging: {
			...
			console: {
				...
				timestamps: {
					doc: 'Whether to add timestamps to the console logging.',
					format: Boolean,
					default: false
				},
			...
			file: {
				...
				timestamps: {
					doc: 'Whether to add timestamps to the file logging.',
					format: Boolean,
					default: true
				},
				level: CONVICT_LOG_LEVEL,
	...
	if (userCfgExists) {
		...
		convictSchema.login = {
			...
			twitch: {
				...
				allowedIds: {
					doc: 'Which Twitch IDs to allow. Can be obtained from [https://twitchinsights.net/checkuser](https://twitchinsights.net/checkuser)',
					format: VALIDATE_STRING_ARRAY,
			...
			discord: {
				enabled: {
					doc: 'Whether to enable Discord authentication.',
					format: Boolean,
					default: false
				},
				clientID: {
					doc: 'A Discord application ClientID https://discord.com/developers/applications/',
					format: String,

					// This will throw if the user does not provide a value, but only if Discord auth is enabled.
					default: userConfig.login && userConfig.login.discord && userConfig.login.discord.enabled ? null : ''
				},
				clientSecret: {
					doc: 'A Discord application ClientSecret https://discord.com/developers/applications/',
					format: String,
				...
				scope: {
					doc: 'A space-separated string of Discord application permissions.',
					format: String,
					default: 'identify'
				},
				allowedUserIDs: {
					doc: 'Which Discord IDs to allow',
					format: VALIDATE_STRING_ARRAY,

					// This will throw if the user does not provide a value and is not using allowedGuilds, but only if Discord auth is enabled.
					default: userConfig.login && userConfig.login.discord && userConfig.login.discord.enabled && !userConfig.login.discord.allowedGuilds ? null : []
				},
				allowedGuilds: {
					doc: 'Which servers to allow users from',
					format: 'discord-guild-array',
					default: userConfig.login && userConfig.login.discord && userConfig.login.discord.enabled && !userConfig.login.discord.allowedUserIDs ? null : [],
					children: {
						guildID: {
							doc: 'Users in this Discord Server are allowed to log in',
							format: String,
							default: userConfig.login && userConfig.login.discord && userConfig.login.discord.enabled && userConfig.login.discord.allowedGuilds ? null : ''
						},
						allowedRoleIDs: {
							doc: 'Additionally require one of the roles on the server to log in',
							format: VALIDATE_STRING_ARRAY,
							default: []
						},
						guildBotToken: {
							doc: 'Discord bot token, needed if allowedRoleIDs is used',
							format: String,
