		...
		convictSchema.login = {
			...
			discord: {
				...
				allowedIDs: {
					...
					// This will throw if the user does not provide a value and is not using requiredGuildIDs, but only if Discord auth is enabled.
					default: userConfig.login && userConfig.login.discord && userConfig.login.discord.enabled && !userConfig.login.discord.allowedGuildIDs ? null : []
				},
				allowedGuildIDs: {
					doc: 'Users in these Discord Guilds are allowed to login',
					format: VALIDATE_STRING_ARRAY,

					// This will throw if the user does not provide a value and is not using allowedIDs, but only if Discord auth is enabled.
					default: userConfig.login && userConfig.login.discord && userConfig.login.discord.enabled && !userConfig.login.discord.allowedIDs ? null : []
				},
				guildRequiredPermissions: {
					doc: 'In addition to being in one of allowedGuildIDs, the user must have these permissions in the guild.',
					format: VALIDATE_STRING_ARRAY,

					// By default, the user must not have any permissions in the guild
					default: []
				}
			},
			local: {
