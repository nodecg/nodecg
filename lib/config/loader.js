...
module.exports = function (userCfgPath) {
	...
	if (fs.existsSync(userCfgPath)) {
		...
		if (userConfig.sentry && userConfig.sentry.enabled) {
			schema.properties.sentry = {
				type: 'object',
				properties: {
					enabled: {
					...
					dsn: {
						description: 'Your private DSN, for server-side error reporting.',
						type: 'string'
					},
					publicDsn: {
						description: 'Your public sentry DSN, for browser error reporting.',
						type: 'string'
					}
				},
				required: ['dsn', 'publicDsn']
			};
		}
