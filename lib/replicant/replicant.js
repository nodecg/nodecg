...
const dereferenceSchema = deasync($RefParser.dereference.bind($RefParser));
...
class Replicant extends EventEmitter {
	...
	constructor(name, namespace, opts = {}) {
		...
		if (opts.schemaPath) {
			const fullSchemaPath = path.join(process.env.NODECG_ROOT, opts.schemaPath);
			if (fs.existsSync(fullSchemaPath)) {
				try {
					this.schema = dereferenceSchema(fullSchemaPath, {
						dereference: {
							circular: false
						}
					});
