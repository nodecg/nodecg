...
function replaceRefs(obj, currentFile, allFiles) {
	...
	if (type.isPOJO && isFileReference(obj)) {
		...
		const referenceFile = allFiles.find(file => {
		...
		delete obj.$ref;
		for (const key in referenceFile.data) {
			if (key === '$schema') {
				continue;
			}
			obj[key] = clone(referenceFile.data[key]);
		}

		// Crawl this POJO or Array, looking for nested JSON References
		const keys = Object.keys(referenceFile.data);
