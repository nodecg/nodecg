...
function NodeCG(bundle, socket) {
	...
	if (process.env.browser) {
		...
		if (window._ncgHasSounds) {
			this._soundCues = new Replicant('soundCues', this.bundleName, {}, socket);
			this._soundFiles = new Replicant('uploads:sounds', this.bundleName, {}, socket);
			...
			this._soundCues.on('change', () => {
				_updateInstanceVolumes.bind(this)();
				_registerSounds.bind(this)();
			});
			this._soundFiles.on('change', _registerSounds.bind(this));
			this._bundleVolume.on('change', _updateInstanceVolumes.bind(this));
			this._masterVolume.on('change', _updateInstanceVolumes.bind(this));
		}
...
NodeCG.readReplicant = function (name, bundle, cb) {
	...
	if (process.env.browser) {
		window.socket.emit('readReplicant', {name, bundle}, cb);
	} else {
		const replicant = replicator.find(name, bundle);
		if (replicant) {
...
if (process.env.browser) {
	...
	if (window._ncgHasSounds) {
		 ...
		 * @returns {Object} - A SoundJS AbstractAudioInstance.
		 */
		NodeCG.prototype.playSound = function (cueName, {updateVolume} = {updateVolume: true}) {
			...
			// Set the volume.
			this._soundCues.value.some(cue => {
				if (cue.name === cueName) {
					_setInstanceVolume.bind(this)(instance, cue);
					return true;
				}
...
const _registeredSums = new Set();
function _registerSounds() {
	this._soundCues.value.forEach(cue => {
		if (!cue.file) {
			return;
		}

		if (!_registeredSums.has(cue.file.sum)) {
			createjs.Sound.registerSound(`${cue.file.url}?sum=${cue.file.sum}`, cue.name, {channels: 100});
			_registeredSums.add(cue.file.sum);
		}
	});

	if (!this.soundsReady) {
		this.soundsReady = true;
		document.dispatchEvent(new CustomEvent('ncgSoundsReady'));
	}
}

function _setInstanceVolume(instance, cue) {
	instance.volume = cue.volume * this._bundleVolume.value * this._masterVolume.value;
}

function _updateInstanceVolumes() {
