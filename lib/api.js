...
NodeCG.Replicant = function (name, bundle, opts) {
	return new Replicant(name, bundle, opts, process.browser ? window.socket : null);
};
...
function _setInstanceVolume(instance, cue) {
	instance.volume = (this._masterVolume.value / 100) * (this._bundleVolume.value / 100) * (cue.volume / 100);
}
