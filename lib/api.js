...
function NodeCG(bundle, socket) {
	...
	if (process.browser) {
		...
		if (globalThis.window && bundle._hasSounds && window.createjs && createjs.Sound) {
		...
		socket.on('message', data => {
			...
			cg._messageHandlers.forEach(handler => {
		...
		socket.on('error', err => {
			if (err.type === 'UnauthorizedError') {
				...
				cg.log.error('Unhandled socket error:', err);
			}
		});
	} else {
		io = server.getIO();

		io.setMaxListeners(75); // Prevent console warnings when many extensions are installed
...
NodeCG.Replicant = function (name, namespace, opts) {
	return new Replicant(name, namespace, opts, process.browser ? globalThis.socket : null);
};
...
NodeCG.readReplicant = function (name, namespace, cb) {
	...
	if (process.browser) {
		globalThis.socket.emit('replicant:read', {name, namespace}, cb);
	} else {
