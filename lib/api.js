...
/**
 * Lets you easily wait for a Replicant to finish declaring.
 *
 * Returns a promise which is resolved once the provided Replicant
 * have emitted a `change` event, which is indicates that it must
 * have finished declaring.
 *
 * This method is only useful in client-side code.
 * Server-side code never has to wait for Replicants.
 *
 * @param {Replicant} replicant
 * @returns {Promise<Replicant>} the declared Replicant.
 *
 * @example <caption>From a graphic or dashboard panel:</caption>
 * const rep = nodecg.Replicant('rep');
 *
 * NodeCG.waitForReplicant(rep).then(() => {
 *     console.log('rep is fully declared and ready to use!');
 * });
 */
NodeCG.waitForReplicant = function (replicant) {
 ...
 * Server-side code never has to wait for Replicants.
 *
 * @param {...(Replicant|Replicant[])} replicants
 * @returns {Promise<Replicant[]>} the declared Replicants.
 *
 * @example <caption>From a graphic or dashboard panel:</caption>
 ...
 * });
 *
 * // You can also provide an array of Replicants.
 * NodeCG.waitForReplicants([rep1, rep2]).then(() => {
 *     console.log('rep1 and rep2 are fully declared and ready to use!');
 * });
 */
NodeCG.waitForReplicants = function (...replicants) {
	return Promise.all(replicants.reduce((a, b) => a.concat(b), []).map(this.waitForReplicant));
};
 ...
 * });
 */
NodeCG.prototype.waitForReplicant = NodeCG.waitForReplicant;
 ...
 * });
 */
NodeCG.prototype.waitForReplicants = NodeCG.waitForReplicants;
