...
NodeCG.readReplicant = function (name, bundle, cb) {
	...
	if (process.browser) {
		window.socket.emit('replicant:read', {name, bundle}, cb);
	} else {
...
if (process.browser) {
	...
	NodeCG.prototype.playSound = function (cueName, opts) {
		...
		const cue = this._soundCues.value.find(cue => cue.name === cueName);
		if (!cue) {
	...
	NodeCG.prototype.stopSound = function (cueName) {
		...
		if (!this._soundCues.value.find(cue => cue.name === cueName)) {
			throw new Error(`Cue "${cueName}" does not exist in bundle "${this.bundleName}"`);
		}
