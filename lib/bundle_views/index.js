...
app.get('/viewsetup.js', function(req, res) {
    ...
    res.render('lib/includes/viewsetup.ejs', {host: config.host, port: config.port, bundleName: bundleName});
});
...
app.get('/view/:view_name*', function(req, res) {
    ...
    if (resName.endsWith('.jade') || resName.endsWith('.ejs') || resName.endsWith('.html')) {
