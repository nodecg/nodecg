...
app.get('/panel/:bundle/:panel', ncgUtils.authCheck, function(req, res, next) {
    ...
    if (panel.dialog) {
        ncgUtils.injectScripts(panel.html, 'dialog', {createApiInstance: bundle, standalone: req.query.standalone}, function(html) {
            res.send(html);
        });
    } else {
        ncgUtils.injectScripts(panel.html, 'panel', {createApiInstance: bundle, standalone: req.query.standalone}, function(html) {
            res.send(html);
        });
    }
});
