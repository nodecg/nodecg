    ...
    window.initPackery = function () {
        ...
        if (storedSortOrder) {
            ...
            for (var i = 0, len = packery.items.length; i < len; i++) {
                ...
                elementName = item.element.getAttribute('element');
                allPanels[i] = elementName;
                itemsByElement[elementName] = item;
            }
            ...
            for (; i < len; i++) {
                elementName = removededOld[i];
                packery.items[i] = itemsByElement[elementName];
            }
        }
        ...
        forEach(panelsList, function (i, itemElem) {
            ...
            var draggie = new Draggabilly(itemElem, {handle: 'nodecg-dashboard-panel .heading'});
        ...
        function orderItems() {
            ...
            for (var i = 0; i < itemElems.length; i++) {
                sortOrder[i] = itemElems[i].getAttribute('element');
            }
