	...
	if (bundle.soundCues.length > 0) {
		...
		if (cuesRep.value.length > 0) {
			...
			defaultCuesRepValue.forEach(defaultCue => {
				...
				if (existingIndex >= 0) {
					cuesRep.value[existingIndex].assignable =
						defaultCue.assignable;
		...
		new Replicant(`volume:${bundle.name}`, '_sounds', {
			defaultValue: 100
		});
	}
});
...
function _serveDefault(req, res) {
	...
	sendFile(parentDir, fullPath, res);
}
