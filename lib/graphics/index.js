...
app.get('/graphics/:bundleName*', (req, res, next) => {
...
function setupBowerComponents(bundles) {
	bundles.forEach(bundle => {
		...
		app.use(`/graphics/${name}/components`, express.static(filepath));
		app.use(`/graphics/${name}/bower_components`, express.static(filepath));
	});
}

Bundles.on('allLoaded', setupBowerComponents);
