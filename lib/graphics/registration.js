...
const {injectScripts, isChildOf} = require('../util');
const io = require('../server').getIO();
const Replicant = require('../replicant');

const BUILD_PATH = path.resolve(__dirname, '../../src/instance');
...
app.get('/instance/*', (req, res, next) => {
	...
	if (resName.endsWith('.html')) {
		const fileLocation = path.resolve(BUILD_PATH, resName);
		if (isChildOf(BUILD_PATH, fileLocation)) {
			injectScripts(fileLocation, 'graphic', {}, html =>
				res.send(html)
			);
		} else {
