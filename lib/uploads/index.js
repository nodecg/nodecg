...
var debounceChanges = require('./debounceChanges');
...
bundles.all().forEach(function (bundle) {
	if (bundle.uploads && bundle.uploads.enabled) {
		...
		foldersToWatch.push(bundleUploadsPath + '/**/*');
	}
});

var watcher = chokidar.watch(foldersToWatch, {
	ignored: /[\/\\]\./
});
...
watcher.on('change', function fileChanged(filepath) {
	debounceChanges(filepath, function () {
		md5File(filepath, function (err, sum) {
			if (err) {
				log.error(err);
				return;
